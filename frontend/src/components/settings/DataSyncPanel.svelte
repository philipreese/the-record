  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-base font-light text-theme-muted mt-2 leading-relaxed">
        Synchronize your local playback records with your ListenBrainz account history. Note: The
        local database acts as a permanent, cumulative archive—deleting items on ListenBrainz will
      ...
      <div class="flex flex-col gap-6">
          ...
          <div class="memory-surface-nested flex flex-col justify-between gap-6 bg-transparent">
            <div class="space-y-3">
              ...
              <p class="text-base font-light text-theme-secondary leading-relaxed">
                Scan for recent tracks since your last sync. Safe and fast. Recommended for daily
                updates.
              </p>
            </div>
            <div>
        ...
        {#if !appCache.isSyncing && appCache.syncStatus?.finished && !appCache.syncError}
          <div class="text-xs text-success font-semibold flex items-center gap-1.5 animate-fade-in">
            ...
            Synced {appCache.syncStatus.synced_count} new play{appCache.syncStatus.synced_count ===
            1
              ? ''
              : 's'} successfully ({appCache.syncStatus.batches_fetched} batch{appCache.syncStatus
              .batches_fetched === 1
              ? ''
              : 'es'}).
          </div>
